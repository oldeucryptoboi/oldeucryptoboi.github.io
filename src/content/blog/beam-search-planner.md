...
I fixed this by giving the planner a second opinion.

## The Inspiration: Probabilistic Dreaming

A paper caught my eye at ICLR 2026 -- [Probabilistic Dreaming for World Models](https://arxiv.org/abs/2603.04715) by Gavin Wong (Yale). The core insight is deceptively simple: in reinforcement learning, maintaining just K=2 hypotheses about the world state gives a **4.5% improvement** and **28% lower variance** compared to a single trajectory. Why? Because a single Gaussian latent collapses mutually exclusive futures into a non-existent average -- what Wong calls "averaging 'left' and 'right' paths into an impossible 'middle' path."

The agent literally freezes when faced with two valid strategies, because its single representation can't hold both.

I realized my LLM planner has the exact same problem.

## The Problem: Mode Collapse in Planning

[KarnEvil9](https://github.com/oldeucryptoboi/KarnEvil9) is my agentic runtime -- it takes a task, generates a plan (a sequence of tool calls), executes it, observes results, and replans. The planner is an LLM that produces structured JSON plans. One call, one plan, one strategy.
...
Inspired by ProbDreamer, I built a `BeamPlanner` that wraps any existing planner as a decorator. The algorithm:
...
- Tool count x task length (many tools + long task = complex)
...
I ran three test cases, each mapping to a failure mode identified in the ProbDreamer paper:
...
|---|---|---|
| Strategy | Browser-only (nav -> scrape x 2) | **HTTP API-first** (wttr.in + TimeOut + fallback) |
| Steps | 5 | 4 |
...
| Paper Prediction | What I Observed |
|---|---|
...
I considered three approaches before landing on contrastive generation:
...
This is version 1. Some ideas I'm exploring:

- **Scoring with execution feedback** -- Right now the scorer uses static heuristics. After a plan executes, I could update scoring weights based on what actually worked. The ProbDreamer paper's free energy scoring (`V(h,z) + B*s^2`) suggests weighting by predicted value plus epistemic uncertainty.
...
- **Ensemble disagreement for complexity classification** -- The paper found that ensemble disagreement collapsed too quickly to be useful for curiosity. But for *classifying* whether a task needs beam search, brief ensemble probing might work.
